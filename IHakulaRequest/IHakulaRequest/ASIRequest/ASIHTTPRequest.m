...
@property (strong) NSArray *responseCookies;
@property (assign) int responseStatusCode;
@property (strong) NSMutableData *rawResponseData;
@property (strong, nonatomic) NSDate *lastActivityTime;
...
@property (strong, nonatomic) NSInputStream *postBodyReadStream;
...
@property (strong, nonatomic) NSOutputStream *fileDownloadOutputStream;
...
@property (strong) NSString *authenticationRealm;
@property (strong) NSString *proxyAuthenticationRealm;
@property (strong) NSString *responseStatusMessage;
...
@property (strong, nonatomic) NSInputStream *readStream;
...
@property (strong) NSNumber *requestID;
@property (weak, nonatomic) NSString *runLoopMode;
@end
...
+ (void)performSelector:(SEL)selector onTarget:(id)target withObject:(__unsafe_unretained id)object amount:(void *)amount
{
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([[self delegate] respondsToSelector:[self didReceiveDataSelector]]) {
            ...
            __unsafe_unretained ASIHTTPRequest *me = self;
			[invocation setArgument:&me atIndex:2];
			__unsafe_unretained NSData *data = [NSData dataWithBytes:buffer length:bytesRead];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	[newRequest setRequestHeaders:[[self requestHeaders] copyWithZone:zone]];
	[newRequest setRequestCookies:[[self requestCookies] copyWithZone:zone]];
...
+ (NSString *)defaultUserAgentString
{
    ...
    NSLog(@"%@", [NSString stringWithFormat:@"%@ %@ (%@; %@ %@; %@)", appName, appVersion, deviceName, OSName, OSVersion, locale]);
...
+ (void)measureBandwidthUsage
{
	...
	maxBandwidthPerSecond
		...
		if (bytesRemaining < 0) {
			...
			throttleWakeUpTime = [bandwidthMeasurementDate dateByAddingTimeInterval:extraSleepyTime];
			#else
...
+ (unsigned long)maxUploadReadLength
{
	...
	if (toRead == 0 || !bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		throttleWakeUpTime = [bandwidthMeasurementDate copy];
	}
	[bandwidthThrottlingLock unlock];	
